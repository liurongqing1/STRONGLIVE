...
def path(skip,scale,name):
    hr = sorted(
        glob.glob(os.path.join('/home/srteam/lrq/EDSR-PyTorch/dataset/benchmark/{}/LR_bicubic/X{}'.format(name,str(scale)), '*' + ".png"))
    )


    for i in range(1, len(hr) + 1, int(skip) + 1):  ######
        print("skip:" + str(i))  ######
        path = os.path.join('/home/srteam/lrq/EDSR-PyTorch/experiment/{}/results-{}/X{}/{}/{}{}'.format(name,name,str(scale),str(skip), str(i), ".png"))
        #print(path)
        img = np.array((Image.open(path)))
    
        t0 = time.time()
        new_img = BiCubic_interpolation(img, img.shape[0] * scale, img.shape[1] * scale)
        diff = time.time() - t0
...
parser.add_argument('--name', type=str, default='Ready',help='test dataset name')
args = parser.parse_args()
diffs = path(args.skip,args.scale,args.name)
pint(diffs)
'''