...
class Trainer():
    ...
    def test(self):
        ...
        nvEnc = nvc.PyNvEncoder({"codec": "h264", 's': '3840x2160', 'preset': 'P4',"profile": "high","tuning_info": "high_quality",
                                 "bitrate": "10M", "fps": "30", "gop": str(skip)}, 0,
                                nvc.PixelFormat.NV12) #
        nvEnc2 = nvc.PyNvEncoder({"codec": "h264", 's': '3840x2160', 'preset': 'P4',"profile": "high","tuning_info": "high_quality",
        ...
        while True:
            ...
            #self.ckp.write_log('tensor: {}s\n'.format(src_tensor.shape))
        ...
        for idx_data, d in enumerate(self.loader_test):
            for idx_scale, scale in enumerate(self.scale):  # 0,4
                ...
                for lr, hr, filename in tqdm(d, ncols=80):  # 创建一个进度条来跟踪迭代的进度
                    ...
                    if num <= 30 and num % skip != 0:
                        sr = core.imresize(lr.squeeze(0), self.scale[0])
                    ...
                    if success:
                        ...
                        print(encFrame.shape)#byteArray
                    ...
                    success_bic = nvEnc.EncodeSingleSurface(bic_surface, encFrame, sync=True)  # False，则编码操作是异步的
                    #trans: 0.413s

                    # print(encFrame.device)
                    if num % skip == 0 and success_sr:
                        #print("sr:",str(num), str(encFrame2.shape))  # byteArray
                        dstFile.write(bytearray(encFrame2))  ##前3帧出不来
                        framesReceived += 1
                    if num % skip != 0 and success_bic:
                        #print("bic:",str(num), str(encFrame.shape))
                        dstFile.write(bytearray(encFrame))  ##前3帧出不来
                        framesReceived += 1
                    #trans: 0.006s
                    '''
                    ################异步插帧.3###############
                    ...
                    if num == 0:
                        if success_sr:
                            #print("sr:", str(num), str(encFrame2))  # byteArray
                            pkt.append(encFrame2)  # .copy()
                            # print(pkt)
                            framesReceived += 1
                    if num >3 and (num-3) % skip == 0:
                        if success_sr:
                            #print("sr:", str(num-3), str(encFrame2))  # byteArray
                            pkt.append(encFrame2)  # .copy()
                            # print(pkt)
                            framesReceived += 1
                    ...
                    if success_bic:
                        if num >3 and (num-3) % skip != 0 and num != 3:
                            #print("bic:", str(num-3), str(encFrame))
                    ...
                    '''
                    while True:
                        success_sr = nvEnc2.FlushSinglePacket(encFrame2)
                        success_bic = nvEnc.FlushSinglePacket(encFrame)
                        if num % skip == 0 and success_sr and (framesReceived < 30):
                            byteArray2 = bytearray(encFrame2)
                            print(" flush sr:",str(num), str(encFrame2.shape))
                            dstFile.write(byteArray2)
                            framesReceived += 1
                            framesFlushed += 1
                        if num % skip != 0 and success_bic and (framesReceived < 30):
                            byteArray = bytearray(encFrame)
                            print("flush bic:",str(num), str(encFrame.shape))
        ...
        for i in N:
            print(i)
            if success_sr and i %skip ==0  and (framesReceived < 30):
                pkt[i]=encFrame2
                #print("flush sr："+str(i))
                print(encFrame2)
                #print(pkt)
                framesReceived += 1
                framesFlushed += 1
        for i in N:
            print(i)
            if success_bic and i %skip !=0 and (framesReceived < 30):
                pkt[i]=encFrame
                #print("flush bic："+str(i))
                print(encFrame)
                #print(pkt)
                framesReceived += 1
                framesFlushed += 1
